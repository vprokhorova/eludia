	...
	if ($options -> {label}) {
		...
		$_REQUEST{__script} .= <<EOJS;
			function on_load() {
				...
				if (window.name == 'invisible') {
					parent.alert(data[0], null, { on_close: function() { redirect(); }, title: '¬ÌËÏ‡ÌËÂ'});
				} else {
					alert(data[0], null, { on_close: function() { redirect(); }, title: '¬ÌËÏ‡ÌËÂ'});
				}
			}
EOJS
...
sub draw_text_cell {
	...
	$data -> {attributes} -> {class} .= ' row-cell-overflow'
...
<html>
	<head>
		...
		<script>
			...
			function load () {
				var a = $a;
				if (a.alert) alert (a.alert, null, {title: '¬ÌËÏ‡ÌËÂ'});
